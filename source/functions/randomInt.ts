...
function handleMaxMin(min: number, max: number, maxInclusive: boolean) {
    ...
    if (Number.MAX_SAFE_INTEGER === min && maxInclusive) throw new Error('Min integer can only be maximum MAX_SAFE_INTEGER - 1')
...
export function randomIntSync(min: number, max: number, options?: RandomIntegerOptions) {
...
export async function randomInt(min: number, max: number, maxInclusive?: boolean) {
